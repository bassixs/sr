...
const presets: Record<GalleryPreset, { eyebrow: string; title: string; text: string; order: string[]; filters: GalleryFilter[] }> = {
  about: {
    ...
    order: ['grounds', 'lobby', 'event-hall', 'library', 'pool'],
    filters: [
  ...
  stay: {
    ...
    order: ['room-1', 'room-2', 'grounds', 'dining-room', 'library'],
    filters: [
...
export function SanatoriumGallery({ preset = 'about' }: { preset?: GalleryPreset }) {
  ...
  const sortCategories = (items: GalleryCategory[]) => [...items].sort((left, right) => {
    ...
    const rightIndex = config.order.indexOf(right.id);
    return (leftIndex < 0 ? 999 : leftIndex) - (rightIndex < 0 ? 999 : rightIndex);
  });
  const availableCategories = useMemo(() => sortCategories(categories.filter((category) => config.filters[0].match(category.id))), [categories, config]);
  const selectedCategories = useMemo(() => sortCategories(categories.filter((category) => selectedFilter.match(category.id))), [categories, selectedFilter, config]);
